...
document.querySelectorAll('.gallery img, .gallery svg').forEach(elem => {
    console.log(elem);
    elem.onclick = () => {
        if (elem.parentElement.nodeName == 'FIGURE') {
            elem.parentElement.classList.toggle('plus');
        }
        elem.classList.toggle('plus');
    }
});
